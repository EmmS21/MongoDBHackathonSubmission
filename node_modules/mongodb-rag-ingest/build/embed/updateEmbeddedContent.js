"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.updateEmbeddedContentForPage = exports.updateEmbeddedContent = void 0;
const mongodb_rag_core_1 = require("mongodb-rag-core");
const crypto_1 = require("crypto");
const chunkPage_1 = require("./chunkPage");
/**
  (Re-)embeddedContent the pages in the page store that have changed since the given date
  and stores the embeddedContent in the embeddedContent store.
 */
const updateEmbeddedContent = async ({ since, embeddedContentStore, pageStore, sourceNames, embedder, chunkOptions, }) => {
    const changedPages = await pageStore.loadPages({
        updated: since,
        sources: sourceNames,
    });
    mongodb_rag_core_1.logger.info(`Found ${changedPages.length} changed pages since ${since}${sourceNames ? ` in sources: ${sourceNames.join(", ")}` : ""}`);
    for (const page of changedPages) {
        switch (page.action) {
            case "deleted":
                mongodb_rag_core_1.logger.info(`Deleting embedded content for ${page.sourceName}: ${page.url}`);
                await embeddedContentStore.deleteEmbeddedContent({
                    page,
                });
                break;
            case "created": // fallthrough
            case "updated":
                await (0, exports.updateEmbeddedContentForPage)({
                    store: embeddedContentStore,
                    page,
                    chunkOptions,
                    embedder,
                });
        }
    }
};
exports.updateEmbeddedContent = updateEmbeddedContent;
const chunkAlgoHashes = new Map();
const getHashForFunc = (f, o) => {
    const data = JSON.stringify(o ?? {}) + f.toString();
    const existingHash = chunkAlgoHashes.get(data);
    if (existingHash) {
        return existingHash;
    }
    const hash = (0, crypto_1.createHash)("sha256");
    hash.update(data);
    const digest = hash.digest("hex");
    chunkAlgoHashes.set(data, digest);
    return digest;
};
const updateEmbeddedContentForPage = async ({ page, store, embedder, chunkOptions, }) => {
    const contentChunks = await (0, chunkPage_1.chunkPage)(page, chunkOptions);
    if (contentChunks.length === 0) {
        // This could happen if source returned a page with no content
        mongodb_rag_core_1.logger.warn(`No content for page ${page.sourceName}:${page.url} - deleting any existing content and continuing`);
        await store.deleteEmbeddedContent({ page });
        return;
    }
    // In order to resume where we left off (in case of script restart), compare
    // the date of any existing chunks with the page updated date. If the chunks
    // have been updated since the page was updated (and we have the expected
    // number of chunks) and chunkAlgoHash has not changed from what's in the
    // database, assume the embedded content for that page is complete and
    // up-to-date. To force an update, you can delete the chunks from the
    // collection.
    const existingContent = await store.loadEmbeddedContent({
        page,
    });
    const chunkAlgoHash = getHashForFunc(chunkPage_1.chunkPage, chunkOptions);
    if (existingContent.length &&
        existingContent[0].updated > page.updated &&
        contentChunks.length === existingContent.length &&
        existingContent[0].chunkAlgoHash === chunkAlgoHash) {
        mongodb_rag_core_1.logger.info(`Embedded content for ${page.sourceName}:${page.url} already updated (${existingContent[0].updated}) since page update date (${page.updated}). Skipping embedding.`);
        return;
    }
    mongodb_rag_core_1.logger.info(`${page.action === "created" ? "Creating" : "Updating"} embedded content for ${page.sourceName}:${page.url}`);
    const embeddedContent = [];
    // Process sequentially because we're likely to hit rate limits before any
    // other performance bottleneck
    for (let i = 0; i < contentChunks.length; ++i) {
        const chunk = contentChunks[i];
        mongodb_rag_core_1.logger.info(`Vectorizing chunk ${i + 1}/${contentChunks.length} for ${page.sourceName}: ${page.url}`);
        const { embedding } = await embedder.embed({
            text: chunk.text,
        });
        embeddedContent.push({
            ...chunk,
            embedding,
            updated: new Date(),
            chunkAlgoHash,
        });
    }
    await store.updateEmbeddedContent({
        page,
        embeddedContent,
    });
};
exports.updateEmbeddedContentForPage = updateEmbeddedContentForPage;
//# sourceMappingURL=updateEmbeddedContent.js.map